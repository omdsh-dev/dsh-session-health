...
import { scanZstdFrames } from './zstd-scan.ts'
import { enumerateSessions, resolveDshHome, resolveSessionPath, sessionsRoot, type SessionFile } from './files.ts'
import { buildReport, countIssues, type FileDiagnosis, type HealthReport } from './report.ts'
import { deepAnalyze, type DeepResult } from './deep.ts'

export const name = '@deepseek-ai/dsh-session-health'
export const inject = ['tools']

interface SessionHealthArgs {
  action: string
  path?: unknown
  deep?: unknown
  detail?: unknown
...
/** 帧级诊断单个文件。 */
export async function diagnoseFile(f: SessionFile, now = Date.now()): Promise<FileDiagnosis> {
  const base: FileDiagnosis = {
  ...
  switch (scan.error) {
    case 'not-zstd': base.issues.push('not-zstd'); return base
    case 'reserved-header': base.issues.push('reserved-header'); return base
    case 'reserved-block': base.issues.push('reserved-block'); return base
    case 'truncated': base.issues.push('torn'); return base
  }
  if (scan.frames === 1) {
    if (f.bytes > OVERSIZED_SINGLE_FRAME_BYTES) base.issues.push('oversized-single-frame')
    if (now - f.updatedAt > EMPTY_SESSION_AGE_MS) base.issues.push('empty-session')
  }
  return base
}
...
async function runAction(args: SessionHealthArgs): Promise<string> {
  ...
  if (args.action === 'scan') {
    ...
    const shown = detailRequested
      ...
      : []
    report.detail = shown as unknown as FileDiagnosis[]
    for (const w of warnings) report.suggestions.push(w)
  ...
  if (args.action === 'file' || args.action === 'stats') {
    ...
    const kind: SessionFile['kind'] = target.endsWith('.jsonl.zstd')
      ? 'zstd'
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    name: 'session_health',
    description:
      'Diagnose dsh session files under $DSH_HOME/sessions (multi-frame zstd scan: ' +
    ...
    execute: args => runAction(args as SessionHealthArgs),
    timeoutMs: 5000,
  }))
}
